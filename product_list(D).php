...
if (isset($_GET['classid'])){
    // 使用產品類別查詢
    $queryFirst = sprintf("SELECT * FROM product,product_img WHERE p_opne=1 AND product_img.sort=1 AND product.p_id=product_img.p_id AND product.classid='%d' ORDER BY product.p_id DESC", $_GET['classid']);
//這段老師還沒教，自己先打上去的
}else{
    //列出產品product資料查詢
